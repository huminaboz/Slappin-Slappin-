...
{   
    ...
    public void DrawGraph(SO_GrowthCurve growthCurve, int pointsCount)
    {
        foreach (Vector2 point in growthCurve.GetGraphPoints(pointsCount))
        {
            CreatePoint(point);
        }
        UpdateGraph();
    ...
    public void Initialize(Canvas newCanvas)
    {
        if(CheckForErrors())
            return;
